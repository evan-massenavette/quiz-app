<template>
  <v-container class="pa-0">
    <v-form
      v-model="form"
      @submit.prevent="$emit('edited', question)"
      class="my-5"
    >
      <v-container>
        ...
        <v-img
          :src="question.image"
          :class="question.image ? 'ma-auto mb-5' : 'd-none'"
          style="height: 200px"
        />
        ...
        <div id="controls_container" class="d-flex flex-row justify-center">
          ...
          <v-btn-toggle
            ...
            divided
            mandatory
            id="good_answer_selector"
            class="controls_subcontainer"
          >
          ...
          <v-card-actions class="controls_subcontainer">
            <v-btn
              :disabled="loading || (!form && !existent)"
              :loading="loading"
              type="submit"
              color="accent"
              >{{ existent ? "Edit" : "Add" }}</v-btn
            >

            <v-btn
              v-if="existent"
              :disabled="loading || !canGoUp"
              @click="$emit('go-up', question)"
              icon="mdi-arrow-collapse-up"
            ></v-btn>
            <v-btn
              v-if="existent"
              :disabled="loading || !canGoDown"
              @click="$emit('go-down', question)"
              icon="mdi-arrow-collapse-down"
            ></v-btn>
            <v-btn
              v-if="existent"
              :disabled="loading"
              @click="$emit('deleted', question)"
              icon="mdi-close"
            ></v-btn>
          </v-card-actions>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      goodAnswer: this.currentQuestion.possibleAnswers.findIndex(

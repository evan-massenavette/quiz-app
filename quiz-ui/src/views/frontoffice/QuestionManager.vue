  ...
  <v-container id="main_card_wrapper" fluid>
    <v-card id="main_card">
      ...
      <QuestionDisplay
        ...
        :currentAnswer="answers[currentQuestionPosition - 1]"
...
export default {
  ...
  methods: {
    ...
    async endQuiz() {
      const postScoreRequest = await QuizApiService.postScore(
    ...
    async loadQuestionByPosition() {
      if (
        ...
        // Load question if it's possible
        const questionRequest = await QuizApiService.getQuestion(
          this.currentQuestionPosition
        );
